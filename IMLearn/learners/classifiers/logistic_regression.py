...
class LogisticRegression(BaseEstimator):
    ...
    def build_weights(self, n):
        return np.random.standard_normal(n) / np.sqrt(n)
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "l1":
            module = RegularizedModule(LogisticModule(), L1(), lam=self.lam_, include_intercept=self.include_intercept_)
        elif self.penalty_ == "l2":
            module = RegularizedModule(LogisticModule(), L2(), lam=self.lam_, include_intercept=self.include_intercept_)
        else:
            module = LogisticModule()
        module.weights = self.build_weights(X.shape[1])
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        # Now change the prediction to 1 and 0
        predictions[predictions > self.alpha_] = 1
        predictions[predictions <= self.alpha_] = 0
        return predictions
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:
            X = np.c_[X, np.ones(len(X))]
        denominator = 1 + np.exp(-X @ self.coefs_)
